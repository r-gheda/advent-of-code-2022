...
fn main() {
    ...
    let mut min = -1;
    let sys_size = folder_size.get("/");
    match sys_size
    {
        Some(x) => {
            let needed_space = 30000000 - (70000000 - *x);
            min = *x;
            for (_key, size) in folder_size {if min > size && size >= needed_space {min = size;}}
        },
        None => panic!("Base folder size not found"),
    }
